...
namespace MyJournal.Components.Pages
{
    ...
    public partial class CreateEditEntry
    {
        ...
        private void InsertMarkdown(string snippet)
        {
            ...
            {
                entry.Content = $"{entry.Content}{Environment.NewLine}{snippet}";
            }
        }

        private void InsertBold() => InsertMarkdown("**bold**");
        private void InsertItalic() => InsertMarkdown("*italic*");
        private void InsertHeading() => InsertMarkdown("# Heading");
        private void InsertListItem() => InsertMarkdown("- List item");
        private void InsertLink() => InsertMarkdown("[Link text](https://example.com)");

        private MarkupString GetPreviewHtml()
        {
            try
            {
                return (MarkupString)MarkdownService.ToHtml(entry.Content);
            }
            catch
            {
                return (MarkupString)MarkdownService.ToHtml(MarkdownService.ToPlainText(entry.Content));
            }
        }
    }
}
