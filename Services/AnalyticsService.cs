...
namespace MyJournal.Services
{
    ...
    public class AnalyticsService
    {
        ...
        public async Task<AnalyticsData> GetAnalyticsAsync(DateTime? startDate = null, DateTime? endDate = null)
        {
            ...
            CalculateCategoryAnalytics(entries, analytics);
        ...
        private void CalculateTagAnalytics(List<JournalEntry> entries, AnalyticsData analytics)
        {
            ...
            if (totalEntries > 0)
            {
                analytics.TagPercentages = tagCounts.ToDictionary(
                    kvp => kvp.Key,
                    kvp => Math.Round((double)kvp.Value / totalEntries * 100, 2)
                );
            }
        }

        private void CalculateCategoryAnalytics(List<JournalEntry> entries, AnalyticsData analytics)
        {
            var categoryCounts = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);

            foreach (var entry in entries)
            {
                var category = string.IsNullOrWhiteSpace(entry.Category) ? "General" : entry.Category.Trim();
            ...
            if (total > 0)
            {
                analytics.CategoryPercentages = categoryCounts.ToDictionary(
                    kvp => kvp.Key,
